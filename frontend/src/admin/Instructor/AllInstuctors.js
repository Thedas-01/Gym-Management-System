...
const[keyword,setKeyword]=useState("");
  //Search Equipment
  const search=()=>{
    axios.get(`http://localhost:8020/instructor/search/${keyword}`).then((response)=>{
    ...
    await axios
      ...
      .then(res => {
        if (res.data) {
          ...
          data = res.data 
          const headers = [["ID", "FIRST NAME", "LAST NAME", "AGE", "GENDER","EMAIL","CONTACT"]];
          const datas = res.data.map(elt => [++count, elt.first_name, elt.last_name, elt.age, elt.gender,elt.email,elt.contact]);
          let content = {
            startY: 50,
            head: headers,
            body: datas
