...
//Get One using ID
  ...
  //Search Equipment
  const search=()=>{
    axios.get(`http://localhost:8020/equipment/search/${keyword}`).then((response)=>{
      console.log(response);
  ...
  //Download Report 
    const savePDF = async () => {
      ...
      doc.setFontSize(15);
      doc.text("Equipment Report", 40, 40);
      var data
      var price = 0
      var count = 0
      ...
      await axios
        ...
        .then(res => {
          if (res.data) {
            ...
            const datas = res.data.map(elt => [++count, elt.Id, elt.name, elt.type, new Date(elt.YOM).getFullYear(),elt.dimension,new Date(elt.last_service_date).toLocaleDateString(
              "en-US",
              { day: "numeric", month: "numeric", year: "numeric" }
            ),new Date(elt.next_service_date).toLocaleDateString(
              "en-US",
              { day: "numeric", month: "numeric", year: "numeric" }
            )]);
            let content = {
