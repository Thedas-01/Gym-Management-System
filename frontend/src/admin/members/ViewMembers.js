...
export default function AllMembers() {
 ...
 const[keyword,setKeyword]=useState("");
    //Search Members
    const search=()=>{
      axios.get(`http://localhost:8020/user/search/${keyword}`).then((response)=>{
    ...
    await axios
      ...
      .then(res => {
        if (res.data) {
          ...
          data = res.data 
          const headers = [["ID", "Name", "Age", "Gender", "Email","Contact"]];
          const datas = res.data.map(elt => [++count, elt.name, elt.age, elt.gender, elt.email,elt.contact]);
          let content = {
            startY: 50,
            head: headers,
            body: datas
